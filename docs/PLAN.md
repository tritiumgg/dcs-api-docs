...
protocol, and **mise** as the task runner. The binary is `dcs-api-docs`; project commands are
...
| A10 | Raw executor replies live in a configured directory outside the repository, defaulting to `../dcs-api-docs-captures`; nothing in the repository reads it at build time and nothing in this plan reads it at all | 2.4, 6.6 |
...
| 1.2 | `DcsApiDocsCensus.lua`: this project's own resident instrument, one source, `HOST` = `hook` or `export`, built once per host, every build loaded by the harness | `mise run instrument:build` prints `hosts: N/N loaded under Lua 5.1.5`; mutation: a 5.4-only token in the source → red | 0.2 | dev |
...
| 4.6 | Live session A — `parked-standin-sp` at the menu: census of `gui`, `hook`, `config`, `scripting`, each at SP-7's budget | `dcs-api-docs status` prints `runs: N complete, controls: N/N pass, frontier: 0 ×N`; `equivalence census gui` prints `0`; every observation file tracked | 4.1, 3.7, SP-7 | DCS+human |
| 4.7 | Live session B — `parked-standin-sp` with the fixture mission loaded and paused: census of `mission`, `missionscripting` through `a_do_script`, `export` from both sides, and a further run re-censusing `hook` at `mission-paused`, so that §6.8's two-phases-of-one-state rule has a measured case to agree or split on | `dcs-api-docs status` prints `runs: N complete` (`mission`, `missionscripting`, `export/hook`, `export/export`, `hook@mission-paused`), `controls: N/N pass` | 4.4, 4.5, 4.6 | DCS+human |
| 4.8 | Live session C — `unparked-sp`: tier-2 index over every cited candidate on the `mission` dispatcher tables, crashes handled by 3.5 | `dcs-api-docs status` prints `runs: n (complete c, crashed k), candidates: N, covered: N, killers: K`; every candidate cites its source | 4.2, 5.5, 4.7 | DCS+human |
...
| no session left parked | `dcs-api-docs status` → `parks outstanding: 0` |
...
| tier 2 covered its list | `dcs-api-docs status` → `candidates: N, covered: N` |
...
| both denominators printed, and per scenario | `dcs-api-docs coverage` → `a/b`, `c/d`, the per-field split, one line per scenario |
...
| R2.2 | Live session D — `parked-standin-mp-client`: every state SP-2 reached, at the phase each needs | `dcs-api-docs status` prints `runs: n complete, controls: n/n pass`; `equivalence` over one capability prints `0` | R2.1, 4.7 | DCS+human |
...
| R3.2 | Live session E — `parked-standin-mp-server`: every state SP-1 reached, with the fixture mission hosted | `dcs-api-docs status` prints `runs: n complete, controls: n/n pass`; `equivalence` over one capability prints `0` | R3.1, R2.2 | DCS+human |
