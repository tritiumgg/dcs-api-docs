...
#
# A comment reading `model.md §4.3` sends the reader to a frozen document the
# build is drifting away from, to find a reason that would have fitted in the
# comment. A comment reading `task 6.2` points at nothing once the plan
...
CITE='(model|findings|captures|benchmark)\.md[[:space:]]*(§|#)|SPEC[[:space:]]*§|§[0-9]+\.[0-9]'
TASK='\b[Tt]ask[[:space:]]+([0-9]+\.[0-9]+[a-z]?|B\.[0-9]+[a-z]?|SP-[0-9]+|C\.[0-9]+|R[0-9]\.[0-9]+)\b|\bdone-when\b'
...
for f in $files; do
    case "$f" in
        docs/*|CLAUDE.md|README.md|tools/nospecrefs.sh|.claude/hooks/*|.claude/skills/*|.claude/agents/*) continue ;;
    esac
    [ -f "$f" ] || continue
