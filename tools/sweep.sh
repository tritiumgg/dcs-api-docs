...
# the instrument will not load, and that line is what is looked for here.
load_failed() {
    grep -qF 'harness: cannot load the instrument' "$work/out.log"
}
...
while IFS="$US" read -r id kind cmd reddens controls reason; do
    ...
    if [ "$rc" -eq 0 ]; then
        ...
        if [ -z "$failed" ]; then
            ...
            notbuilt="exit $rc and the instrument would not load"
            said=$(printf '%s\n' "$failed" | head -1)
        fi
