...
- command: `sh -c 'if grep -q moved alpha.txt; then echo "FAIL  fixture/whatever: raised: harness: cannot load the instrument: unexpected symbol"; exit 1; fi; exit 0'`
...
#
# Which rows are green comes from the gates. The sandbox supplies that as a
# `status` file, one `<id><TAB>green` per line, which is what the gate reads
# in place of the verify runner under --root.
#
...
{
    printf '### Stage 9 — a fixture stage\n\n'
    printf '| id | task | done when | needs | runs on |\n|---|---|---|---|---|\n'
    printf '| %s | a fixture with a control | it holds; mutation: break it and it does not | — | dev |\n' "$one"
    printf '| %s | a fixture with no control | it holds | — | dev |\n' "$two"
    printf '| %s | another fixture with a control | it holds; mutation: break it | — | dev |\n' "$three"
} > "$cover/docs/PLAN.md"

inventory_for() {
    : > "$cover/docs/mutations.md"
    for t in "$@"; do
        printf '### fixture/%s\n- task: %s\n- command: `true`\n- reddens: never\n\n' "$t" "$t" >> "$cover/docs/mutations.md"
    done
}

# Nothing green, nothing written: the truth before the verify runner exists.
inventory_for
rm -f "$cover/status"
out=$(sh "$cover/tools/sweep-cover.sh" --root "$cover" 2>&1) && got=0 || got=$?
check 'nothing green owes nothing' 0 "$got" "0 green rows owe an entry" "$out"

# A green row that names a mutation and has no entry is the finding.
printf '%s\tgreen\n' "$one" > "$cover/status"
out=$(sh "$cover/tools/sweep-cover.sh" --root "$cover" 2>&1) && got=0 || got=$?
check 'a green row with a mutation and no entry is refused' \
    1 "$got" "row $one is green and names a mutation, and docs/mutations.md has no entry" "$out"

# The same row with its entry passes, and a green row naming no mutation owes nothing.
inventory_for "$one"
printf '%s\tgreen\n%s\tgreen\n' "$one" "$two" > "$cover/status"
out=$(sh "$cover/tools/sweep-cover.sh" --root "$cover" 2>&1) && got=0 || got=$?
check 'an entry per green mutation row passes' 0 "$got" "1 green rows owe an entry, 1 have one" "$out"

# An entry for a row that names no mutation is a stray.
inventory_for "$one" "$two"
out=$(sh "$cover/tools/sweep-cover.sh" --root "$cover" 2>&1) && got=0 || got=$?
check 'an entry for a row with no mutation is a stray' \
    1 "$got" "names row $two, which names no mutation" "$out"

# A row not yet green owes nothing even though its entry may already exist.
inventory_for "$one" "$three"
out=$(sh "$cover/tools/sweep-cover.sh" --root "$cover" 2>&1) && got=0 || got=$?
check 'an entry written early for a row not yet green is not a stray' \
    0 "$got" "1 green rows owe an entry, 1 have one" "$out"
