...
# What is staged is what a commit records, whether it was staged by name or
# by `git add -A`. Raw replies, logs, crash dumps and supervisor zips never
# enter the repository: they hold paths with the operator's username and are
# not the evidence tier this project tracks. The shell guard refuses a
# `git add` that names one; this catches one that arrived under `-A`.
staged=$(git diff --cached --name-only 2>/dev/null || true)
bad=$(printf '%s\n' "$staged" | grep -E '\.(res|log|dmp)$|(^|/)captures?/|(^|/)supervisor/.*\.zip$' || true)
[ -z "$bad" ] || report "staged files that never enter the repository:
$bad
Unstage them with git restore --staged <path>. Raw replies live outside the
repository (docs/specs/captures.md); logs and dumps are session residue."
