...
# The specifications freeze when the build starts, marked by docs/specs/.frozen.
# Before that they are working documents and are edited in place.
# SPECS_FROZEN=1 lets the hook test exercise the frozen branch without the file.
frozen=no
...
case "$norm" in
    */docs/specs/*|docs/specs/*)
        [ "$frozen" = yes ] && why="a frozen specification" ;;
